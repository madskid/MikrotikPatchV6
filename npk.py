...
class NpkInfo:
    ...
    def unserialize_from(data: bytes) -> 'NpkInfo':
        assert len(data) == struct.calcsize(
            NpkInfo._format), 'Invalid data length'
        _name, _version, _build_time, unknow = struct.unpack_from(
            NpkInfo._format, data)
...
class NpkNameInfo(NpkInfo):
    ...
    def unserialize_from(data: bytes) -> 'NpkNameInfo':
        assert len(data) == struct.calcsize(
            NpkNameInfo._format), 'Invalid data length'
        _name, _version, _build_time, _unknow = struct.unpack_from(
            NpkNameInfo._format, data)
...
class NpkFileContainer:
    ...
    def serialize(self) -> bytes:
        ...
        for item in self._items:
            data = struct.pack(self._format, item.perm, item.type, item.usr_or_grp, item.modify_time, item.revision,
                               item.rc, item.minor, item.major, item.create_time, item.unknow, len(item.data), len(item.name))
    ...
    def unserialize_from(data: bytes):
        ...
        while len(decompressed_data):
            ...
            data = decompressed_data[offset +
                                     name_size:offset+name_size+data_size]
            items.append(NpkFileContainer.NpkFileItem(perm, type, usr_or_grp, modify_time,
                         revision, rc, minor, major, create_time, unknow, name, data))
        ...
        while offset < len(data):
            ...
            if self._has_pkg:
                if part_id == NpkPartID.NAME_INFO:
                    ...
                    self._packages[-1]._parts.append(NpkPartItem(
                        NpkPartID(part_id), NpkNameInfo.unserialize_from(part_data)))
                else:
                    self._packages[-1]._parts.append(
                        NpkPartItem(NpkPartID(part_id), part_data))
            else:
                if part_id == NpkPartID.NAME_INFO:
                    self._parts.append(NpkPartItem(
                        NpkPartID(part_id), NpkNameInfo.unserialize_from(part_data)))
                elif part_id == NpkPartID.PKG_INFO:
                    self._parts.append(NpkPartItem(
                        NpkPartID(part_id), NpkInfo.unserialize_from(part_data)))
                else:
                    self._parts.append(NpkPartItem(
                        NpkPartID(part_id), part_data))
    ...
    def sign(self, kcdsa_private_key: bytes, eddsa_private_key: bytes):
        ...
        if len(self._packages) > 0:
            ...
            for package in self._packages:
                is_legacy_signature = len(package[NpkPartID.SIGNATURE].data) == 68
                if is_legacy_signature:
                    package[NpkPartID.SIGNATURE].data = b'\0'*68
                elif len(package[NpkPartID.SIGNATURE].data) != 20+48+64:
            ...
            is_legacy_signature = len(self[NpkPartID.SIGNATURE].data) == 68
            if is_legacy_signature:
                self[NpkPartID.SIGNATURE].data = b'\0'*68
            elif len(self[NpkPartID.SIGNATURE].data) != 20+48+64:
            ...
            print(f"Signing Single Package SHA1: {sha1_digest.hex().upper()}")
            sha256_digest = self.get_digest(hashlib.new('SHA256'))

            if is_legacy_signature:
                kcdsa_signature = mikro_kcdsa_sign(sha1_digest, kcdsa_private_key)
                print(f"Generated Sig: {kcdsa_signature.hex().upper()}")
                self[NpkPartID.SIGNATURE].data = sha1_digest + kcdsa_signature
            else:
                kcdsa_signature = mikro_kcdsa_sign(sha256_digest[:20], kcdsa_private_key)
                eddsa_signature = mikro_eddsa_sign(sha256_digest, eddsa_private_key)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='nova package creator and editor')
    ...
    create_option_parser = subparsers.add_parser(
        'create', help='Create option.npk file')
    ...
    create_option_parser.add_argument(
        'squashfs', type=str, help='NPK squashfs file')
    create_option_parser.add_argument(
        '-desc', '--description', type=str, help='NPK description')
    ...
    if args.command == 'sign':
        ...
        option_npk[NpkPartID.DESCRIPTION].data = args.description.encode(
        ) if args.description else args.name.encode()
